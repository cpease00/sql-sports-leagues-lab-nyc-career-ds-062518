...
def select_all_games_date_and_info_teams_name_and_score_for_teams_in_nhl():
   return "SELECT games.date, games.info, teams.name, team_games.score FROM games JOIN team_games ON team_games.game_id = games.id JOIN teams ON teams.id = team_games.team_id JOIN leagues ON leagues.id = teams.league_id WHERE leagues.name = 'NHL'";
